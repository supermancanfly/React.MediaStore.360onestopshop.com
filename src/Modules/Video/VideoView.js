...
const View = () => {
    ...
    const realVideoData = megaList.filter(item => item.type === "mp4");

    const totalPages = Math.ceil(realVideoData?.length / itemsPerPage);

    const startIndex = itemsPerPage * (currentPage - 1);
    const endIndex = startIndex + itemsPerPage;
    const currentItems = realVideoData?.slice(startIndex, endIndex);
    ...
    return (
      ...
      /> */}
                ...
                <div className='flex justify-center items-center mt-10'>
                    <p className='text-blue-400 text-3xl font-bold ml-3'>Video Home</p>
                </div>
